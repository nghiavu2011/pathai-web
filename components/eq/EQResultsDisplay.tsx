...
const EQResultsDisplay: React.FC<EQResultsDisplayProps> = (props) => {
  ...
  const systemInstruction = `Bạn là Trợ lý AI Tham vấn Rèn luyện Trí tuệ Cảm xúc PathAI (Mô hình Daniel Goleman). Dựa trên kết quả tự đánh giá EQ của học sinh, hãy phân tích ý nghĩa và đưa ra gợi ý rèn luyện kỹ năng giao tiếp, làm việc nhóm tích cực (${EQ_DETAILS[lowestSkill.key]}). Kết quả: ${JSON.stringify(results)}`;
